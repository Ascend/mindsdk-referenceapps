...
对于车牌检测模型，在绝大多数情况下都能将车牌正确框选出来，车牌检测准确率较高；但**受限于训练车牌识别模型的数据集**，车牌识别只能识别蓝底车牌，对于黄底车牌和绿底新能源车牌很难正确识别，而且模型对中文字符的识别准确率偏低，对于大角度车牌的识别准确率偏低，对分辨率较低的图片的识别准确率偏低。若用户需要高准确率的结果，可自行训练识别模型，参考该样例实现模型的推理及后处理开发。
