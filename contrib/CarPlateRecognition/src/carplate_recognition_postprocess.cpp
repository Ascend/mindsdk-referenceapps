...
APP_ERROR CarPlateRecognitionPostProcess::DeInit(){
    LogDebug << "Start to DeInit CarPlateRecognitionPostProcess";
    LogDebug << "End to DeInit CarPlateRecognitionPostProcess";
    return APP_ERR_OK;
}
APP_ERROR CarPlateRecognitionPostProcess::Process(std::vector<MxBase::TensorBase> &inferOutputs, MxBase::ObjectInfo &objectInfo)
{
    LogDebug << "Start to Process CarPlateRecognitionPostProcess";
    std::string carPlateRes;
    for (size_t i = 0; i < inferOutputs.size(); i++)
    {
        inferOutputs[i].ToHost();
        auto *output = static_cast<float *>(inferOutputs[i].GetBuffer());
    ...
    objectInfo.className=carPlateRes;
