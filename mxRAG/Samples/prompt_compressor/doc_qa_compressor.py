...
class DocQaCompressor(object):
    def __init__(self, model_path: str, device: int):
        self.pre_precessor = DocSplitter()
        self.core_precessor = LocalReranker(model_path=model_path, device=device)
        self.post_precessor = DocRetriever(model_path=model_path)
    ...
    def run_doc_qa(self, context: str, question: str, target_tokens: int, target_rate: float):
        sentences_list = self.pre_precessor.split_text(text=context)
        ranker_result = self.core_precessor.rerank(query=question, texts=sentences_list)
        dp = DocRetrieverPara(None, sentences_list, ranker_result, target_tokens, target_rate, True)
        compressed_context = self.post_precessor.assemble_result(dp)

        return compressed_context

    def run_structured_doc_qa(self, raw_data: List, question: str, topk: int = 5):
        data_dict = self.structured_post_processor.split_text(raw_data=raw_data)
        ranker_result = self.core_precessor.rerank(query=question, texts=list(data_dict.keys()))
