...
yunet基于VisionSDK开发，在昇腾芯片上进行人脸检测，并实现可视化呈现。输入3路视频，对其进行推理，输出推理结果。
...
**步骤5：** 拷贝so文件至VisionSDK安装路径的`lib/modelpostprocessors/`目录下。在根目录下执行命令：
...
**步骤2：** 拷贝so文件至VisionSDK安装路径的`lib/modelpostprocessors/`目录下。在根目录下执行命令：
