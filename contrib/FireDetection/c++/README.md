...
### 1.1 简介
高速公路车辆火灾识别基于 VisionSDK 开发，在 Atlas 300V、Atlas 300V Pro 上进行目标检测。项目主要流程为：通过ffmpeg打开本地视频文件、模拟视频流，然后进行视频解码，解码结果经过模型推理进行火灾和烟雾检测，如果检测到烟雾和火灾则在日志中进行告警。解码后的视频图像会编码保存至指定位置。
...
* ffmpeg需要下载源码、手动编译。请用户参考ffmpeg官网及相关网络教程进行操作。
