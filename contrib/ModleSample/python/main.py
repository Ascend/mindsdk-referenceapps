...
def process():
    ...
    inputShapeVector  = model.input_shape(0) # 获得模型输入的对应Tensor的数据shape信息。
    inputShapeList = list(inputShapeVector)
    print("input Tensor shape list:", inputShapeList)

    outputShapeVector   = model.output_shape(0) # 获得模型输出的对应Tensor的数据shape信息。
    outputShapeList = list(outputShapeVector)
    print("output Tensor shape list:", outputShapeList)

    inputDtype = model.input_dtype(0)
    print("Input dtype:", inputDtype)

    # 使用numpy生成输入数据 真实情况下读入图片进行推理也可以通过numpy转换为Tensor类型
    img = np.random.randint(0, 255, size=(inputShapeList[0], inputShapeList[1], inputShapeList[2], inputShapeList[3]), dtype=np.uint8).astype(np.float32) #这里的size根据模型输入的要求确定
