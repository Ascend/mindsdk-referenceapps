...
APP_ERROR SsdVggPostProcess::Process(const MxBase::Image &originImage, std::vector<MxBase::TensorBase> &inferOutputs,
    std::vector<MxBase::ObjectInfo> &objectInfos)
{
    ...
    // 将数据从Device侧转移到Host侧
    inferOutputs[0].ToHost();
    inferOutputs[1].ToHost();
    int *objectNum = static_cast<int *>(inferOutputs[0].GetBuffer());
    float *objectInfo = static_cast<float *>(inferOutputs[1].GetBuffer());
