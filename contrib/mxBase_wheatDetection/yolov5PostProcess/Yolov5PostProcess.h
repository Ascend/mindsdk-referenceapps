...
const int DEFAULT_YOLO_VERSION = 5;
...
public:
    ...
    Yolov5PostProcess(const Yolov5PostProcess &other);
...
protected:
    ...
    void CodeDuplicationSetDet(MxBase::ObjectInfo &det, float& x, float& y, float& width, float& height);
    void SelectClassNCHW(std::shared_ptr<void> netout, NetInfo info, 
                         std::vector<MxBase::ObjectInfo>& detBoxes,
                         int stride, OutputLayer layer);
    void SelectClassNHWC(std::shared_ptr<void> netout, NetInfo info, 
                         std::vector<MxBase::ObjectInfo>& detBoxes,
                         int stride, OutputLayer layer);
    void SelectClassNCHWC(std::shared_ptr<void> netout, NetInfo info, 
                         std::vector<MxBase::ObjectInfo>& detBoxes,
                         int stride, OutputLayer layer);
