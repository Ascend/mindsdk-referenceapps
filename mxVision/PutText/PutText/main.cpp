...
APP_ERROR DrawCaption(MxBase::Tensor img) {
    ...
    auto length2 = captionImpl1.getLength("2023-08-20 15：21：06        ")；
    ...
    if (ret != APP_ERR_OK) {
        LogError << "Fail to put the address text in the top right corner.";
        return APP_ERR_COMM_FAILURE;
    }
    // 在图片右上角添加时间信息
    ret = captionImpl2.putText(img, "", "预留信息：分辨率xxxx", cv::Point(textRealSize, H - 3 * textRealSize), 0.3);
    if (ret != APP_ERR_OK) {
        LogError << "Fail to put the address text in the left bottom corner.";
        return APP_ERR_COMM_FAILURE;
    }
    return APP_ERR_OK;
}
...
int main() {
    MxBase::MxInit();
    {
        ...
        APP_ERROR ret = imageProcessor.Decode(imgPath, decodedImage, ImageFormat::BGR_888);
        if (ret != APP_ERR_OK) {
        ...
        ret = MxBase::Tensor imgTensor = decodedImage.ConvertToTensor(true, false);
        if (ret != APP_ERR_OK) {
            LogError << "Fail to convert image to tensor.";
            return -1;
        }
        ...
        // Tensor对象转Image对象
        ret = Image::TensorToImage(imgTensor, decodedImage, "./output.jpg");
        if (ret != APP_ERR_OK) {
