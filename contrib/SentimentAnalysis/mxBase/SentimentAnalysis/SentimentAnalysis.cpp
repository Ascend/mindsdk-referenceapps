...
APP_ERROR SentimentAnalysis::InitTokenMap(const std::string &vocabTextPath, std::map<std::string, int> &tokenMap) {
  const std::string text;
  std::ifstream infile;
  // Open label file.
  infile.open(vocabTextPath, std::ios_base::in);
  if (infile.fail()) {
    LogError << "Failed to open vacab file: " << vocabTextPath << ".";
...
// Load the label file.
APP_ERROR SentimentAnalysis::LoadLabels(const std::string &labelPath, std::map<int, std::string> &labelMap) {
  std::ifstream infile;
  // Open label file.
...
APP_ERROR SentimentAnalysis::Init(const InitParam &initParam) {
...
APP_ERROR SentimentAnalysis::DeInit() {
  model_->DeInit();
...
APP_ERROR SentimentAnalysis::TextToTensor(const std::string &text, std::vector<MxBase::TensorBase> &inputs) {
...
APP_ERROR SentimentAnalysis::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR SentimentAnalysis::PostProcess(const std::vector<MxBase::TensorBase> &outputs, std::string &label) {
  uint32_t i;
  uint32_t maxIndex = 0;
  float maxValue = 0;
...
APP_ERROR SentimentAnalysis::WriteResult(std::string &label) {
  std::ofstream ofs;
...
APP_ERROR SentimentAnalysis::Process(const std::string &text, std::string &label) {
