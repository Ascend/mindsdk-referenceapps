...
**步骤2**  将VisionSDK开发套件包上传到安装环境的任意路径下（如：“/home/package”）并用cd命令进入套件包所在路径。
...
- 若用户未指定安装路径，软件会默认安装到VisionSDK开发套件包所在的路径。

**步骤6**  安装VisionSDK开发套件包。
...
​            在当前窗口手动执行以下命令，让VisionSDK的环境变量生效。
