...
* NumCpp需要下载源码, 请用户至 https://github.com/dpilger26/NumCpp 进行下载。下载完成后，无需编译，仅需将NumCpp源码目录下的include目录拷贝至项目根目录下的postprocess/include目录（include目录需要手动创建）。
### 1.5 代码目录结构与说明
...
**步骤2** 放入待测图片。将一张图片放在路径``python/test_img``下，命名为 test.jpg（test_img目录需用户自行创建）。
